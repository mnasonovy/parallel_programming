...
void batchMultiplyAndSave(const std::string& folder) {
    ...
    for (int threads = 1; threads <= max_threads; threads *= 2) {
        ...
        std::string threads_folder = folder + "/threads_" + std::to_string(threads);
        fs::create_directories(threads_folder);

        for (int size = 10; size <= 1000; size += 10) {
            Matrix A(size);
            Matrix B(size);

            std::string subfolder = threads_folder + "/" + std::to_string(size) + "x" + std::to_string(size);
            ...
            multiplyAndPrintToFile(A, B, resultFilename, threads);
        }
    }
}
